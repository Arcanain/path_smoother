...
public:
        ...
        : Node("save_path_emcl")
    {
        pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
            "/mcl_pose", 10, std::bind(&SavePath::pose_callback, this, std::placeholders::_1));
        ...
        std::string file_path = home_dir + "/ros2_ws/src/path_smoother/path/seikei_1204_emcl.csv";
    ...
    private:
        void pose_callback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
        {
            ...
            double dist = std::sqrt(std::pow(emcl_pose.position.x - pre_x, 2) + std::pow(emcl_pose.position.y - pre_y, 2));
            if (dist > dist_thread) {
                x.push_back(emcl_pose.position.x);
                y.push_back(emcl_pose.position.y);
                RCLCPP_INFO(this->get_logger(), "x: %lf", emcl_pose.position.x);
                RCLCPP_INFO(this->get_logger(), "y: %lf", emcl_pose.position.y);
                double ds = 0.5;
                ...
                if (times == 5){
                    ...
                    pre_x = emcl_pose.position.x;
                    pre_y = emcl_pose.position.y;
                    times = 0;
                    x.clear();
                    y.clear();
                    rx_.clear();
